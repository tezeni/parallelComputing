...
#else
#define CL_TARGET_OPENCL_VERSION 300 // OPENCL Version
#include <CL/cl.h>
#endif
#include <stdio.h>
#include <stdlib.h>
...
"__kernel void kernelA(__global char* A, __global char* B, int offset) {\n"
"    int i = get_global_id(0);\n"
"    A[i] = B[i + offset];\n"
"}\n";

void measureKernelExecutionTime(cl_context context, cl_device_id device, cl_command_queue queue,
                               cl_program program, cl_kernel kernel, char* A, char* B, int offset) {
    cl_mem bufferA, bufferB;
    cl_int err;

    bufferA = clCreateBuffer(context, CL_MEM_WRITE_ONLY, N * sizeof(char), NULL, &err);
    bufferB = clCreateBuffer(context, CL_MEM_READ_ONLY, N * sizeof(char), NULL, &err);

    err = clEnqueueWriteBuffer(queue, bufferA, CL_TRUE, 0, N * sizeof(char), A, 0, NULL, NULL);
    ...
    printf("Offset: %d\tExecution Time: %.6f seconds\n", offset, executionTime);

    err = clEnqueueReadBuffer(queue, bufferA, CL_TRUE, 0, N * sizeof(char), A, 0, NULL, NULL);
...
int main() {
    ...
    kernel = clCreateKernel(program, "kernelA", &err);

    for (int offset = 0; offset <= MAX_OFFSET; offset++) {
        measureKernelExecutionTime(context, device, queue, program, kernel, A, B, offset);
    }
